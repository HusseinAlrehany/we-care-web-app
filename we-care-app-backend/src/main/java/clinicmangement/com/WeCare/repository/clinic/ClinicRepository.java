package clinicmangement.com.WeCare.repository.clinic;

import clinicmangement.com.WeCare.DTO.ClinicDTOProjection;
...
public interface ClinicRepository extends JpaRepository<Clinic, Integer> {
    ...
    @Query("""
             SELECT new clinicmangement.com.WeCare.DTO.ClinicDetailsViewProjection
             (cl.id,
             cl.clinicMobile,
             cl.address,
             CAST(st.stateName AS string),
             ct.cityName)
             FROM Clinic cl
             JOIN cl.state st
             JOIN cl.city ct
             WHERE cl.doctor.id = :doctorId
           """)
    List<ClinicDetailsViewProjection> getClinicDetailsView(@Param("doctorId")Integer doctorId);
